...
class Post extends Controller {
	...
	*/
	public function form( $id_post = NULL )
	{
		...
		$this->view->listCategory = $objCategoria->listarCategoryByType( Typecategory_Model::NOTICIA );
		...
		else
		{
			...
			$this->view->title = "Editar Post: " . $this->model->getTitle();
	...
	*/
	...
	{
		$img_thumb = base64_decode($img);
		$img_normal = str_replace('/thumb/', '/', $img_thumb);

 		if(file_exists($img_normal)){
 			unlink($img_thumb);
			unlink($img_normal);
			echo 'Deletou: ' . $img;
 		}
		else {
			echo 'Nao deletou ' . $img;
		}
	...
	public function wideimage_ajax()
	{
		...
		for($i = 0; $i < count($tmp_name); $i++)
		{
			...
			if(in_array($ext, $allowedExts))
			{
				...
				// Criar uma terceira imagem para as noticias em destaque
